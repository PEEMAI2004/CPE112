...
int main()
{
    ...
    while (1)
    {
        scanf(" %c", &command); // Add a space before %c to consume any leading whitespace characters
...
void deleteElement(Node **head, Node **tail)
{
    ...
    if (*head != NULL)
    {
        ...
        {
            if ((*head)->data == value) // If the only element has the same data as the given value
            {
                free(*head); // Free the memory of the only element
